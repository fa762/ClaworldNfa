...
export function preferPath(ownerPath: ParticipantPath, autonomyPath: ParticipantPath) {
  if (autonomyPath.claimable > 0n || autonomyPath.entered || autonomyPath.matchesToken) return autonomyPath;
  if (ownerPath.claimable > 0n || ownerPath.entered || ownerPath.matchesToken) return ownerPath;
  return null;
}
...
export function useBattleRoyaleParticipantState(
  ...
  return useMemo(() => {
    ...
    const preferred = preferPath(ownerPath, autonomyPath);
    const hasConflict = ownerPath.entered && autonomyPath.entered && ownerPath.address !== autonomyPath.address;
