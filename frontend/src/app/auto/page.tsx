...
function receiptSummary(status: number, pick: <T,>(zh: T, en: T) => T) {
  if (status === 5) return pick('上一条自治请求已经落到终态失败，需要 operator 级别排查后再重放。', 'The last autonomy request reached a terminal failure state and needs an operator-level read before replay.');
  if (status === 4) return pick('上一条自治请求已经链上执行，后续只需要看 receipt 和 proof。', 'The last autonomy request executed onchain and now sits in the receipt/proof path.');
  if (status === 3) return pick('上一条自治请求已经 finalize，结算闭环完成。', 'The last autonomy request is finalized. Settlement is closed and only audit surfaces remain.');
  if (status === 2) return pick('上一条自治请求已经 synced，正在等执行。', 'The last autonomy request is synced and waiting for execution.');
  if (status === 1) return pick('上一条自治请求已经 fulfilled，正在往 synced 执行推进。', 'The last autonomy request is fulfilled and moving toward synced execution.');
  return pick('上一条自治请求还在等待态，尚未进入 fulfilled 执行阶段。', 'The last autonomy request is still pending and has not crossed into fulfilled execution yet.');
}
  ...
  const autoCards = [
    {
      ...
      value: setup.policy?.enabled
        ? pick(`风险模式 ${setup.policy.riskMode} / 日限额 ${setup.policy.dailyLimit}`, `Risk mode ${setup.policy.riskMode} / daily ${setup.policy.dailyLimit}`)
    ...
    },
    {
      ...
      value: pick(`${permissionCount}/4 项就绪`, `${permissionCount}/4 boundaries ready`),
      icon: Shield,
  ...
  return (
    <>
      <section className="cw-band">
        <div className="cw-band--split">
          <div>
            ...
            <h2 className="cw-section-title">{pick(`${companion.name} 现在能看到真实自治边界。`, `${companion.name} now shows live autonomy boundaries.`)}</h2>
            <p className="cw-muted">
              {pick('这个页面现在会读取 Battle Royale 自治路径的真实 policy、权限和 proof 状态。', 'This page now reads real policy, permission, and proof state for the Battle Royale autonomy path.')}
            </p>
          </div>
          <div className="cw-score">
            <strong>{setup.risk?.emergencyPaused ? pick('暂停', 'Paused') : setup.policy?.enabled ? pick('已上线', 'Live') : pick('待配置', 'Setup')}</strong>
      ...
      <WalletGate
        ...
        detail={pick('directive 签名、claim request 和权限读取都依赖当前钱包和已拥有的 NFA。', 'Directive signing, claim requests, and permission reads all depend on the current wallet and owned NFA. Auto should gate here instead of showing blank setup state.')}
      >
        <OwnedCompanionRail
          title={pick('自治编组', 'Managed roster')}
          subtitle={pick('先切换当前龙虾，再查看它的 policy、ledger 和自治 claim 路径。', 'Switch the active lobster before checking policy, ledger, and autonomy claim path.')}
        />
        ...
        <section className={`cw-panel ${latestReceipt && latestReceiptStatus === 5 ? 'cw-panel--cool' : 'cw-panel--warm'}`}>
          <div className="cw-section-head">
            <div>
              ...
              <h3>
                {latestReceipt
                  ? pick(`请求 #${latestReceipt.requestId?.toString() ?? '-'} ${receiptStatusText(latestReceiptStatus, pick)}`, `Request #${latestReceipt.requestId?.toString() ?? '-'} ${receiptStatusText(latestReceiptStatus, pick)}`)
                  : pick('还没有最近一条 Battle Royale 自治回执', 'No recent Battle Royale autonomy receipt yet')}
              </h3>
              <p className="cw-muted">
                {latestReceipt
                  ? receiptSummary(latestReceiptStatus, pick)
                  : pick('等请求开始落地后，这里会直接给出最新状态、花费、credit 和失败原因，不用先钻原始日志。', 'As requests start to land, this panel will surface the latest state, spend, credit, and failure reason without sending the user into raw logs first.')}
              </p>
            </div>
                ...
                {latestReceipt
                  ? latestReceipt.lastError
                    ...
                    : latestReceipt.reasoningCid
                      ...
                      : pick('这条回执没有可见失败串，继续看 proof 和交易路径即可。', 'This receipt has no exposed failure string. Use the proof and tx path if a deeper audit is needed.')
            ...
            <div className={`cw-list-item ${
              ...
              <span>
                {claimWindow.claimable > 0n && claimWindow.preferredPath?.key === 'autonomy'
                  ? pick(`有一笔可走自治路径的 settled Battle Royale 奖励：第 #${claimWindow.matchId?.toString() ?? '-'} 场 / ${formatCLW(claimWindow.claimable)}。`, `A settled Battle Royale autonomy claim is ready: ${formatCLW(claimWindow.claimable)} from match #${claimWindow.matchId?.toString() ?? '-'}.`)
                  : claimWindow.claimable > 0n && claimWindow.preferredPath?.key === 'owner'
                    ? pick('下一笔 settled reward 在 owner 路径上，不应该走自治 claim request。', 'The next settled reward sits on the owner path, so the autonomy operator should not request this claim.')
                    : claimWindow.hasConflict
                      ? pick('owner 和 autonomy participant 路径冲突，先不要开放 claim。', 'Owner and autonomy participant claim paths disagree. Keep the claim blocked until the participant identity is resolved.')
                      : pick('当前没有自治侧的 settled claim 在等待处理。', 'No autonomy-side settled claim is waiting behind the current receipt stream.')}
              </span>
            </div>
          </div>
        </section>
        ...
        <section className="cw-panel cw-panel--cool">
          ...
          <div className="cw-meter-list">
            <div className="cw-meter-row">
              ...
              <strong>
                {proofs.ledger
                  ? pick(`${proofs.ledger.executedCount} 已执行 / ${proofs.ledger.failedCount} 失败`, `${proofs.ledger.executedCount} executed / ${proofs.ledger.failedCount} failed`)
        ...
        <section className="cw-list">
          <div className="cw-list-item cw-list-item--warm">
            ...
            <span>
              {proofs.ledger
                ? pick(`总花费 ${formatCLW(proofs.ledger.totalActualSpend)} / 总 credit ${formatCLW(proofs.ledger.totalClwCredit)} / ${proofs.ledger.totalXpCredit} XP`, `Total spend ${formatCLW(proofs.ledger.totalActualSpend)} / total credit ${formatCLW(proofs.ledger.totalClwCredit)} / ${proofs.ledger.totalXpCredit} XP`)
                : pick('等 live 自治回执出现后，action hub ledger 会显示在这里。', 'The action hub ledger will appear here once live autonomy receipts exist.')}
            </span>
          </div>
          ...
          <div className={`cw-list-item ${battleRoyaleParticipant.claimable > 0n ? 'cw-list-item--warm' : 'cw-list-item--cool'}`}>
            ...
            <span>
              {claimWindow.claimable > 0n && claimWindow.preferredPath?.key === 'autonomy'
                ? pick(`Battle Royale 自治领取已就绪：第 #${claimWindow.matchId?.toString() ?? '-'} 场 / ${formatCLW(claimWindow.claimable)}。`, `Battle Royale autonomy claim ready: ${formatCLW(claimWindow.claimable)} from settled match #${claimWindow.matchId?.toString() ?? '-'}.`)
                : claimWindow.claimable > 0n && claimWindow.preferredPath?.key === 'owner'
                  ? pick(`这笔 settled reward ${formatCLW(claimWindow.claimable)} 走 owner-wallet 路径。`, `Settled reward ${formatCLW(claimWindow.claimable)} sits on the owner-wallet path for match #${claimWindow.matchId?.toString() ?? '-'}.`)
                  : claimWindow.hasConflict
                    ? pick('Battle Royale 的 claim 数据在 owner / autonomy participant 之间冲突，需要人工排查。', 'Settled Battle Royale claim data conflicts between owner and autonomy participant paths and needs manual review.')
                    : battleRoyaleParticipant.entered && battleRoyaleParticipant.preferredPath?.key === 'autonomy'
                      ? pick(`当前 Battle Royale 参赛走自治 participant 路径，match #${battleRoyale.matchId?.toString() ?? '-'}.`, `Current Battle Royale participation is tracked on the autonomy participant path for match #${battleRoyale.matchId?.toString() ?? '-'}.`)
                      : pick('当前没有自治侧 Battle Royale claim 在等待处理。', 'No active autonomy-side Battle Royale claim is visible for this lobster.')}
            </span>
          </div>
        </section>
      </WalletGate>
