...
export function taskSubmitArgs(nfaId: number, taskType: number, xp: number, clw: number, matchScore: number) {
  return {
    ...
    args: [BigInt(nfaId), taskType, xp, BigInt(Math.round(clw * 1e18)), matchScore] as const,
    gas: 500_000n,
  };
}
...
/** 生成 commit hash: keccak256(abi.encodePacked(strategy, salt)) */
export function generateCommit(strategy: number): { commitHash: `0x${string}`; salt: `0x${string}` } {
  const saltBytes = new Uint8Array(32);
  crypto.getRandomValues(saltBytes);
  const salt = ('0x' + Array.from(saltBytes).map(b => b.toString(16).padStart(2, '0')).join('')) as `0x${string}`;
  const commitHash = keccak256(encodePacked(['uint8', 'bytes32'], [strategy, salt]));
